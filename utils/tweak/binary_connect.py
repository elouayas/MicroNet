...
class BinaryConnect():
    ...
    def save_params(self):
        ...
        for index in range(self.num_of_params):
            ...
            #self.saved_params[index].detach()

            
    def binarization(self):
        ### Save the current full precision parameters using the save_params method
        self.save_params()
        ### Binarize the weights in the model, by iterating through the list of target modules and overwrite the values with their binary version
        for module in self.target_modules:
            module = torch.tensor(module.round(), dtype=torch.int8)
    ...
    def clip(self, bits):
        ...
        for module in self.target_modules:
            ...
            module = torch.tensor(clamp(module, min = -(2**(bits-1) - 1), max = 2**(bits-1)), dtype=torch.int8)

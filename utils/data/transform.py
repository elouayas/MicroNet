...
def random_flip(data):
    """Flip the input data horizontally with 50% probability."""
...
def cutout_numpy(img, size=16):
    ...
    This operation applies a `size`x`size` mask of zeros to a random location within `img`.
    Args:
...
def pil_unwrap(pil_img):
    ...
    index1, index2 = np.where(pic_array[:, :, 3] == 0)
    pic_array = (pic_array[:, :, :3] - MEANS) / STDS
    pic_array[index1, index2] = [0, 0, 0]
    return pic_array
...
class TransformT(object):
    ...
    def pil_transformer(self, probability, level):
        """ applies a transfomration with a given probability """
        def return_function(image):
    ...
    def do_transform(self, image, level):
        """ applies a transformation with  a given level """
        func = self.pil_transformer(PARAMETER_MAX, level)
        return pil_unwrap(func(pil_wrap(image)))
...
IDENTIY = TransformT('identity', lambda pil_img, level: pil_img)
...
def _shear_x_impl(pil_img, level):
    ...
    Args:
        ...
        level: Strength of the operation specified as an Integer from [0, `PARAMETER_MAX`].
    Returns:
...
def _enhancer_impl(enhancer):
    ...
    def impl(pil_img, level):
        value = float_parameter(level, 1.8) + .1  # going to 0 just destroys it
        return enhancer(pil_img).enhance(value)
    return impl
